...
def generate_preview(request: HttpRequest):
    ...
    if not style_id_value and not custom_style_image:
        return JsonResponse(
            {"ok": False, "error": "Select a style or upload a custom haircut image."},
            status=400,
        )
    ...
    if style_id_value:
        ...
        if style is None:
            return JsonResponse({"ok": False, "error": "Selected style is unavailable."}, status=404)
    ...
    with transaction.atomic():
        ...
        if style is not None and one_style_per_session:
            already_used = PlaygroundGeneration.objects.filter(
            ...
            if already_used:
                return JsonResponse(
                    {
                        "ok": False,
                        "error": "This style was already used in the current session.",
                    },
                    status=409,
                )
